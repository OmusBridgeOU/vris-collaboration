...
export function PostcardPrint({ order }: { order: StaffOrderBatch }) {
  ...
  useEffect(() => {
    ...
    async function prepare() {
      try {
        if (!order.items.length) throw new Error("注文に画像がありません。");
        const ready = await Promise.all(
          order.items.map(async (item) => {
            const url = new URL(item.imageUrl, window.location.origin);
            if (
              url.origin !== window.location.origin ||
              !/^\/api\/staff\/order-items\/[^/]+\/print-image$/.test(
                url.pathname,
              )
            )
  ...
  const ready =
    images.length === order.items.length && images.length > 0 && !error;
  ...
  function print() {
    if (!can_print || printing_ref.current) return;
      ...
      {ready ? (
        selected_items.length ? (
          <ul
            ...
            aria-label="注文画像一覧"
          >
            {selected_items.flatMap(({ item, image, remaining_quantity }) =>
              Array.from({ length: remaining_quantity }, (_, copy) => (
                <li key={`${item.id}-${copy}`} className="relative min-w-0">
                  ...
                  <button
                    ...
                    disabled={printing}
                    onClick={() => {
                      if (printing_ref.current) return;
                      set_removed_item_ids((current) => [...current, item.id]);
                    }}
